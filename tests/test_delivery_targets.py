...
class TestConfigurationReasons:
    ...
    def test_frequency_needs_at_least_one_tag(self, db, advertiser):
        campaign = _campaign(db, advertiser, frequency_tags=None)
        assert "frequency_tags" in FrequencyTarget().unconfigured_reason(campaign)
    ...
    def test_missing_credentials_ignores_the_pause_switch(self, db, advertiser):
        """A paused campaign with no tags is misconfigured, not merely paused."""
        paused_broken = _campaign(db, advertiser, delivery_enabled=False, frequency_tags=None)
        assert "frequency_tags" in FrequencyTarget().missing_credentials_reason(paused_broken)
...
class TestFanOut:
    def test_delivers_to_every_configured_target(self, client, db, advertiser, both_available, monkeypatch):
        monkeypatch.setattr("app.delivery.frequency.deliver_ad", lambda r, t, a: "<VAST/>")
    ...
    def test_delivers_to_every_frequency_tag(self, client, db, advertiser, both_available, monkeypatch):
        """A campaign may need more than one Frequency ad unit, so each tag gets its own attempt."""
        tags = [{"token": "tok-a", "app_id": "app-a"}, {"token": "tok-b", "app_id": "app-b"}]
        campaign = _campaign(db, advertiser, dv360_enabled=False, frequency_tags=tags)
        run = _run(db, campaign)

        seen = []
        monkeypatch.setattr(
            "app.delivery.frequency.deliver_ad",
            lambda r, t, a: seen.append(t["app_id"]) or f"<VAST for {t['app_id']}/>",
        )

        deliver_run(run, b"audio")

        assert sorted(seen) == ["app-a", "app-b"]
        assert run.delivery_state == DELIVERY_DELIVERED
        assert {a.detail for a in run.delivery_attempts} == {"app-a", "app-b"}

    def test_one_frequency_tag_failing_leaves_the_other_delivered(self, client, db, advertiser, both_available, monkeypatch):
        """One bad ad unit must not stop the others in the same fan-out."""
        tags = [{"token": "tok-a", "app_id": "app-a"}, {"token": "tok-b", "app_id": "app-b"}]
        campaign = _campaign(db, advertiser, dv360_enabled=False, frequency_tags=tags)
        run = _run(db, campaign)
        ...
        def flaky(ad_run, tag, audio):
            if tag["app_id"] == "app-a":
                raise FrequencyDeliveryError("ad unit rejected it")
            return "<VAST/>"

        monkeypatch.setattr("app.delivery.frequency.deliver_ad", flaky)

        deliver_run(run, b"audio")

        assert run.delivery_state == DELIVERY_PARTIAL
        outcomes = {a.detail: a.succeeded for a in run.delivery_attempts}
        assert outcomes == {"app-a": False, "app-b": True}
